...
def game(state):
    ...
    while not is_winner(state.players):
        ...
        if check_player_in(state.players[turn]):
            ...
            random.shuffle(r)
            for i in r:
                action_challenges = get_action_challenges(action, state.players[i], state.players)
                print("Possible challenges for " + state.players[i].name + ":")
                print(action_challenges)
                action_challenge = state.players[i].agent.choice(action_challenges, "Please choose action challenge from:")
            ...
            if not (action_challenge == None): # if a challenge has occured:
                ...
                random.shuffle(r)
                for i in r:
                    counteractions = get_counteractions(state.players[i], state.players, action)
                    print("Possible counteractions for " + state.players[i].name + ":")
                    print(counteractions)
                    counteraction = state.players[i].agent.choice(counteractions, "Please choose counteraction from:")
                ...
                if counteraction == None: # the action went unchallenged so if no one counteracts the action succeeds 
                    ...
                    random.shuffle(r)
                    for i in r:
                        counteraction_challenges = get_counteraction_challenges(counteraction, state.players[i], state.players)
                        print("Possible challenges to counteraction for " + state.players[i].name + ":")
                        print(counteraction_challenges)
                        counteraction_challenge = state.players[i].agent.choice(counteraction_challenges, "Please choose counteraction challenge from:")
